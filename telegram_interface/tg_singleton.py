...
class TelegramMessagesWrapper:
    ...
    @staticmethod
    def new_suggestion_of_enemies(game: Game):
        ...
        text = prefix + " von " + game.enemy_team.short_name + " für Spieltag " + game.game_day + ":\n"
    ...
    @staticmethod
    def new_suggestion(game: Game):
        msg = "Neuer [Zeitvorschlag von uns](https://www.primeleague.gg/de/leagues/matches/{}) für Spieltag ".format(
            game.game_id) + \
              game.game_day + " gegen " + game.enemy_team.short_name + ". " + EMOJI_SUCCESS
        send_message(msg=msg, chat_id=game.team.telegram_channel_id)

    @staticmethod
    def scheduling_confirmation(game: Game, auto_confirm):
        if auto_confirm:
            text = "Das Team " + game.enemy_team.short_name + " hat für Spieltag " + game.game_day + " weder die vorgeschlagene Zeit angenommen, " + \
            ...
            text = "Spielbestätigung von " + game.enemy_team.short_name + " für Spieltag " + game.game_day + ":\n" + EMOJI_ARROW + \
    ...
    @staticmethod
    def new_lineup_of_enemies(game: Game, ):
        op_link = game.get_op_link_of_enemies(only_lineup=True)
        if op_link is None:
            raise Exception()
        text = game.enemy_team.short_name + " hat ein neues [Lineup]({}) aufgestellt.".format(op_link)
        send_message(msg=text, chat_id=game.team.telegram_channel_id)

    @staticmethod
    def new_game_day(game: Game):
        op_link = game.get_op_link_of_enemies(only_lineup=False)
        text = f"Spieltag {game.game_day} gegen [{game.enemy_team.short_name}]" \
